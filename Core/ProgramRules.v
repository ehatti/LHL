...
Lemma lemOr {T E F} {VE : Spec T E} {VF : Spec T F} {PL PR A QL QR GL GR R} {C : Prog E A} {i} :
  True ->
  VerifyProg (VE:=VE) (VF:=VF) i R GL PL C QL ->
  VerifyProg i R GR PR C QR ->
  VerifyProg i R (GL \4/ GR) (PL \4/ PR) C (QL \5/ QR).
unfold VerifyProg. intros.
rewrite paco_eqv.
cut (
  forall PF,
  PF ==> (PL \4/ PR) ->
  paco_safe i R (GL \4/ GR) PF C (QL \5/ QR)
).
{
  intros. apply H2. easy.
}
...
destruct C; pfold.
{
  ...
  {
    unfold Commit. unfold ReltToPrec.
    intros. psimpl.
    apply H8 in H9.
    assert (ReltToPrec P0 s ρs \/ ReltToPrec P s ρs).
    admit. clear H9.
    destruct H13.
    {
      unfold Commit in H6.
      assert (H9' := H9).
      apply H6 with (t:=t) in H9. psimpl.
      exists x1.
    ...
    }
    {
      unfold Commit in H1.
      assert (H9' := H9).
      apply H1 with (t:=t) in H9. psimpl.
      exists x1.
      split. exists x2. easy.
      split. easy.
      split.
